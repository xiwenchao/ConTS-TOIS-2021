...
r_list = [-1, -0.8, -0.02, 0.5, 1.2] # quit by user ,fail to recommend, ask attribute not accepted, ask attribute accepted, accept recommendation
d = 64
v = 0.01
...
class agent():
    ...
    def response(self, input_message):
        ...
        if input_message.message_type == cfg.REJECT_REC:
            ...
            self.history_list.append(-1)  # 表示推荐失败
            if self.mini == 1:
